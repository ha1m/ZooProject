...
public class AddAnimalDialog extends JDialog implements ItemListener, ActionListener {
    ...
    private int chosenFac;
	/**
	 * Create the dialog, lables and bottuns
	 */
    public AddAnimalDialog(String title,int chosenFactory)
    {
    	super(new JFrame(),title,true);
    	chosenFac=chosenFactory;
    ...
    /**
     * actionPerformed if push on Ok button
     */
    public void actionPerformed(ActionEvent e)
    {
 		if(e.getSource() == ok)
		{
 			...
 			ZooPanel.getInstance().addAnimal(chosenAnimal,sl_sz.getValue(),sl_hor.getValue(),sl_ver.getValue(),c,chosenFac);
		    setVisible(false);
		}
		else 
		    setVisible(false);
